...
mesh = read_vtp("mallasaneurisk/0002.vtp")


#num_file = np.load("reconstruidos-train/0010-eps01.npy")
#num_file = np.load("22-2-beam2/31.npy")
#num_file = np.load("p20/val/0074a_02.npy")
#num_file = np.load("0002_01.npy")
#num_file = np.load("dataset_21_2_2/numpy_preorder/val/0090-original.npy")
#num_file = np.load("reconstruidos-val-augmented/0077_02.npy")
#num_file = np.load("p15/test/0085_01.npy").reshape(-1,39)
#num_file = np.load("generadas/p20-beam1/7.npy")
#num_file = np.load("generadas-p20-orig/0073.npy")
#num_file = np.load("p20-3rot-beam1/p20-3rot-beam1-2/129.npy")
num_file = np.load("62.npy")
all_tck = []
print("all tck", all_tck)
print("shape", num_file.shape)
centerline = num_file[:,:]
print("centerline", np.mean(centerline, axis = 1))
t_all = num_file[:,27:]
c_all = num_file[:,3:27]
print("knots", t_all.shape)
print("coefs", c_all.shape)
#print("centerline", centerline.shape)
print("primeras lineas centerlines", centerline[:15])
...
#serial = str(serial[:-1])
all_spline_points = evaluate_splines(all_tck, num_points=1000)
splines_2 = create_vtk_polydata(all_spline_points)
centerline = serial2centerline(ser)

visualize_vtk_polydata(splines_2, None, None, "Splines Visualization")
visualize_vtk_polydata(centerline, splines_2, None, "Splines Visualization")
